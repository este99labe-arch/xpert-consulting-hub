...
interface InvoiceData {
  ...
  company: {
    /** Nombre comercial: el de la marca si la factura tiene una. */
    name: string;
    /** Razón social del emisor. Se imprime siempre que difiera del comercial:
     *  la factura debe identificar a quién emite, y quien emite es la cuenta,
     *  no la marca. */
    legalName?: string;
    taxId?: string; address?: string; city?: string; postalCode?: string; phone?: string; email?: string;
    logoUrl?: string;
  };
...
async function generatePdf(d: InvoiceData): Promise<Uint8Array> {
  ...
  const drawParty = (
    ...
    const lines: string[] = [];
    /* Si la marca comercial no coincide con la razón social, la factura debe
       decir quién emite de verdad. Solo aplica al emisor. */
    if ((info as any).legalName && (info as any).legalName !== name) lines.push((info as any).legalName);
  ...
  const footerText = `${d.company.name}${d.company.taxId ? ` - NIF/CIF: ${d.company.taxId}` : ""} - ${(d.company as any).footerText || "Documento generado automáticamente"}`;
...
<div class="grid-2"><div class="info-card accent"><div class="info-label">Emisor</div><div class="info-name">${d.company.name}</div><div class="info-text">${d.company.legalName && d.company.legalName!==d.company.name?`${d.company.legalName}<br>`:""}${d.company.taxId?`NIF/CIF: ${d.company.taxId}<br>`:""}${d.company.address?`${d.company.address}<br>`:""}${d.company.postalCode||d.company.city?`${d.company.postalCode||""} ${d.company.city||""}<br>`:""}${d.company.email?d.company.email:""}${d.company.phone?` - ${d.company.phone}`:""}</div></div><div class="info-card"><div class="info-label">Destinatario</div><div class="info-name">${d.client.name}</div><div class="info-text">${d.client.taxId?`NIF/CIF: ${d.client.taxId}<br>`:""}${d.client.address?`${d.client.address}<br>`:""}${d.client.postalCode||d.client.city?`${d.client.postalCode||""} ${d.client.city||""}<br>`:""}${d.client.email?d.client.email:""}${d.client.phone?` - ${d.client.phone}`:""}</div></div></div>
...
<div class="parties"><div class="party"><div class="party-label">De</div><div class="party-name">${d.company.name}</div><div class="party-info">${d.company.legalName && d.company.legalName!==d.company.name?`${d.company.legalName}<br>`:""}${d.company.taxId?`NIF/CIF: ${d.company.taxId}<br>`:""}${d.company.address?`${d.company.address}<br>`:""}${d.company.postalCode||d.company.city?`${d.company.postalCode||""} ${d.company.city||""}<br>`:""}${d.company.email?d.company.email:""}${d.company.phone?` - ${d.company.phone}`:""}</div></div><div class="party"><div class="party-label">Para</div><div class="party-name">${d.client.name}</div><div class="party-info">${d.client.taxId?`NIF/CIF: ${d.client.taxId}<br>`:""}${d.client.address?`${d.client.address}<br>`:""}${d.client.postalCode||d.client.city?`${d.client.postalCode||""} ${d.client.city||""}<br>`:""}${d.client.email?d.client.email:""}${d.client.phone?` - ${d.client.phone}`:""}</div></div></div>
...
function renderHtmlTemplate(template: string, data: InvoiceData, options: any = {}): string {
  switch (template) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [accountRes, settingsRes, paymentsRes, linesRes, brandRes] = await Promise.all([
      supabase.from("accounts").select("name, legal_name, tax_id, phone, email, address, city, postal_code").eq("id", invoice.account_id).single(),
      supabase.from("account_settings").select("invoice_template, invoice_template_options").eq("account_id", invoice.account_id).single(),
      ...
      // La marca de la factura; si no tiene, la principal de la cuenta, que es
      // la que recoge lo que se emitió antes de que existieran las marcas.
      invoice.brand_id
        ? supabase.from("brands").select("*").eq("id", invoice.brand_id).maybeSingle()
        : supabase.from("brands").select("*").eq("account_id", invoice.account_id).eq("is_default", true).maybeSingle(),
    ]);

    const account = accountRes.data;
    const brand = brandRes.data as any;
    /* La plantilla la manda la MARCA; los ajustes de la cuenta quedan como
       respaldo para facturas anteriores a las marcas. */
    const template = brand?.invoice_template || settingsRes.data?.invoice_template || "classic";
    const templateOptions = {
      ...(settingsRes.data?.invoice_template_options || {}),
      ...(brand?.invoice_template_options || {}),
      ...(brand?.legal_footer ? { footerText: brand.legal_footer } : {}),
    };
    ...
    const data: InvoiceData = {
      ...
      company: {
        // Comercial arriba, fiscal debajo: quien recibe la factura reconoce la
        // marca, y Hacienda necesita ver quién la emite.
        name: brand?.name || account?.name || "Empresa",
        legalName: account?.legal_name || account?.name || undefined,
    ...
    const html = renderHtmlTemplate(template, data, templateOptions);
