...
// ─── TEMPLATE RENDERING (duplicated from client for edge runtime) ──────

const fmtMoney = (n: number) => Number(n).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ".");

interface InvoiceData {
  typeLabel: string; invoiceNumber: string; issueDate: string; concept: string; description?: string;
...
<div class="meta-row"><div class="meta-box"><div class="meta-label">Nº Documento</div><div class="meta-value">${d.invoiceNumber}</div></div><div class="meta-box"><div class="meta-label">Fecha de emisión</div><div class="meta-value">${d.issueDate}</div></div><div class="meta-box"><div class="meta-label">Estado</div><div class="meta-value">${d.statusLabel}</div></div></div>
<table><thead><tr><th>Concepto</th><th class="r" style="width:140px">Base imponible</th><th class="r" style="width:100px">IVA (${d.vatPercentage}%)</th><th class="r" style="width:130px">Total</th></tr></thead><tbody><tr><td style="font-weight:600">${d.concept||"—"}</td><td class="r">€${fmtMoney(d.amountNet)}</td><td class="r">€${fmtMoney(d.amountVat)}</td><td class="r b">€${fmtMoney(d.amountTotal)}</td></tr>${d.description?`<tr><td colspan="4" class="desc-cell">${d.description}</td></tr>`:""}</tbody></table>
...
<div class="totals"><div class="totals-box"><div class="t-row"><span>Base imponible</span><span class="mono">€${fmtMoney(d.amountNet)}</span></div><div class="t-row"><span>IVA (${d.vatPercentage}%)</span><span class="mono">€${fmtMoney(d.amountVat)}</span></div><div class="t-divider"></div><div class="t-total"><span>Total</span><span class="mono">€${fmtMoney(d.amountTotal)}</span></div>${totalPaid>0?`<div class="t-row" style="color:#16a34a;font-weight:600"><span>Pagado</span><span class="mono">€${fmtMoney(totalPaid)}</span></div><div class="t-balance"><span>Saldo pendiente</span><span class="mono">€${fmtMoney(balance)}</span></div>`:""}</div></div>
...
<div class="meta-strip"><div class="meta-item"><div class="meta-item-label">Nº Documento</div><div class="meta-item-value">${d.invoiceNumber}</div></div><div class="meta-item"><div class="meta-item-label">Fecha emisión</div><div class="meta-item-value">${d.issueDate}</div></div><div class="meta-item"><div class="meta-item-label">Estado</div><div class="meta-item-value">${d.statusLabel}</div></div></div>
<table><thead><tr><th>Concepto</th><th class="r" style="width:140px">Base imponible</th><th class="r" style="width:100px">IVA (${d.vatPercentage}%)</th><th class="r" style="width:130px">Total</th></tr></thead><tbody><tr><td style="font-weight:600">${d.concept||"—"}</td><td class="r">€${fmtMoney(d.amountNet)}</td><td class="r">€${fmtMoney(d.amountVat)}</td><td class="r b">€${fmtMoney(d.amountTotal)}</td></tr>${d.description?`<tr><td colspan="4" class="desc-cell">${d.description}</td></tr>`:""}</tbody></table>
${d.payments&&d.payments.length>0?`<div class="payments"><h3>Pagos registrados</h3>${d.payments.map(p=>`<div class="pay-row"><span>${p.date} — ${p.method}</span><span style="font-weight:600">€${fmtMoney(p.amount)}</span></div>`).join("")}</div>`:""}
<div class="totals"><div class="totals-box"><div class="t-row"><span>Base imponible</span><span class="mono">€${fmtMoney(d.amountNet)}</span></div><div class="t-row"><span>IVA (${d.vatPercentage}%)</span><span class="mono">€${fmtMoney(d.amountVat)}</span></div><div class="t-total"><span>Total</span><span class="mono">€${fmtMoney(d.amountTotal)}</span></div>${totalPaid>0?`<div class="t-paid"><span>Pagado</span><span class="mono">€${fmtMoney(totalPaid)}</span></div><div class="t-balance"><span>Saldo pendiente</span><span class="mono">€${fmtMoney(balance)}</span></div>`:""}</div></div>
<div class="footer">${d.company.name}${d.company.taxId?` · NIF/CIF: ${d.company.taxId}`:""}<br>Documento generado automáticamente</div>
</div></div></body></html>`;
}
...
<table><thead><tr><th>Concepto</th><th class="r" style="width:130px">Base</th><th class="r" style="width:100px">IVA ${d.vatPercentage}%</th><th class="r" style="width:120px">Total</th></tr></thead><tbody><tr><td style="font-weight:600">${d.concept||"—"}</td><td class="r">€${fmtMoney(d.amountNet)}</td><td class="r">€${fmtMoney(d.amountVat)}</td><td class="r b">€${fmtMoney(d.amountTotal)}</td></tr>${d.description?`<tr><td colspan="4" class="desc-row">${d.description}</td></tr>`:""}</tbody></table>
<div class="rule"></div>
...
<div class="totals"><div class="totals-inner"><div class="t-row"><span>Base imponible</span><span class="mono">€${fmtMoney(d.amountNet)}</span></div><div class="t-row"><span>IVA (${d.vatPercentage}%)</span><span class="mono">€${fmtMoney(d.amountVat)}</span></div><div class="t-total"><span>Total</span><span class="mono">€${fmtMoney(d.amountTotal)}</span></div>${totalPaid>0?`<div class="t-paid"><span>Pagado</span><span class="mono">€${fmtMoney(totalPaid)}</span></div><div class="t-balance"><span>Pendiente</span><span class="mono">€${fmtMoney(balance)}</span></div>`:""}</div></div>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch account, settings, and payments in parallel
    const [accountRes, settingsRes, paymentsRes] = await Promise.all([
