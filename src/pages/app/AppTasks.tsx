...
import {
  CalendarClock, Plus, Search, LayoutGrid, List,
} from "lucide-react";
...
const AppTasks = () => {
  ...
  const handleDragStart = useCallback((e: React.DragEvent, reminderId: string) => {
  ...
  const handleDrop = useCallback((e: React.DragEvent, columnKey: string) => {
    ...
    if (reminderId) {
      updateStatusMutation.mutate({ id: reminderId, status: columnKey });
    }
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="text-left p-3 font-medium text-muted-foreground hidden md:table-cell">Etiquetas</th>
                    <th className="text-left p-3 font-medium text-muted-foreground hidden sm:table-cell">Fecha</th>
                ...
                <tbody>
                  {filtered.map((r: any) => (
                    <tr key={r.id} className="border-b last:border-0 hover:bg-accent/50">
                      ...
                      <td className="p-3 text-xs text-muted-foreground whitespace-nowrap hidden sm:table-cell">
...
const KanbanCard = ({
  ...
  return (
    <Card
      className={`shadow-sm cursor-grab active:cursor-grabbing transition-shadow hover:shadow-md ${isOverdue ? "border-destructive/30" : ""}`}
      draggable
